...
def test_entidades_titulados_anno(df:pd.DataFrame, export:str, display:bool=True) -> None:
...
def test_total_titulados_anno(df:pd.DataFrame, export:str, display:bool=True) -> pd.DataFrame:
    plt.figure(figsize=(10, 6))
    df['TOTAL TITULACIONES ACUM'] = df.groupby('AÑO')['TOTAL TITULACIONES'].cumsum()
    plot = sns.lineplot(data=df, x="AÑO", y="TOTAL TITULACIONES ACUM", marker='o', color='b')
